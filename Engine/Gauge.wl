gaugeFlowWedge[w_, c0_Association, d0_Association, nMax_Integer, tVar_, tDeg_Integer] :=
	gaugeFlowSolve[gaugeDdotWedge, w, c0, d0, nMax, tVar, tDeg]
...
gaugeDdotWedge[d_, w_][i_, j_] := Sum[u w[i + 1 - u] d[u, j], {u, 0, i + 1}] +
...
gaugePhiWedge[cf_][i_, j_] := gaugePhiValueWedge[cf, Min[i, j], Max[i, j]]

gaugePhiValueWedge[cf_, 1, j_] /; 1 <= j <= 6 := 2 cf[0, j + 2]/cf[0, 1]

gaugePhiValueWedge[cf_, 2, 2] := 2 cf[0, 5]/cf[0, 1] + cf[0, 3]^2/cf[0, 1]^2

gaugePhiValueWedge[cf_, 2, 3] := 2 cf[0, 6]/cf[0, 1] + 2 cf[0, 3] cf[0, 4]/cf[0, 1]^2

gaugePhiValueWedge[cf_, 2, 4] := 2 cf[0, 7]/cf[0, 1] + (cf[0, 4]^2 + 2 cf[0, 3] cf[0, 5])/cf[0, 1]^2

gaugePhiValueWedge[cf_, 2, 5] := 2 cf[0, 8]/cf[0, 1] + 2 (cf[0, 4] cf[0, 5] + cf[0, 3] cf[0, 6])/cf[0, 1]^2

gaugePhiValueWedge[cf_, 3, 3] := 2 cf[0, 7]/cf[0, 1] + (2 cf[0, 4]^2 + 2 cf[0, 3] cf[0, 5])/cf[0, 1]^2 + 2 cf[0, 3]^3/(3 cf[0, 1]^3)

gaugePhiValueWedge[cf_, 3, 4] := 2 cf[0, 8]/cf[0, 1] + (4 cf[0, 4] cf[0, 5] + 2 cf[0, 3] cf[0, 6])/cf[0, 1]^2 + 2 cf[0, 3]^2 cf[0, 4]/cf[0, 1]^3

gaugePhiGFWedge[cf_][i_, j_] := With[{s = i + j},
	2 Sum[Coefficient[Coefficient[Expand[gaugePhiGFKernelWedge[cf, s]^r], \[FormalX], i], \[FormalY], j]/r,
		{r, 1, Quotient[s, 2]}]]

gaugePhiGFKernelWedge[cf_, s_] := Expand[\[FormalX] \[FormalY] Sum[(cf[0, m + 1]/cf[0, 1])
	Sum[\[FormalX]^a \[FormalY]^(m - 2 - a), {a, 0, m - 2}], {m, 2, s}]]

gaugePhi[cf_][i_, j_] := -gaugePhiGFWedge[cf][i, j]

gaugeDefect[cf_, df_][i_, j_] := df[i, j] - gaugePhi[cf][i, j]
